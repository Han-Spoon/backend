 ...
 *
 * <p>동일 카드 재저장 시 새 행을 만들지 않고 기존 카드의 저장 시각만 갱신해 목록 최상단으로 올린다.
 * 동일 판정은 타입별 페이로드(템플릿형=ingredients, owner_question=text)로 비교한다.
 */
@Service
public class CardService {
    ...
    public SavedCardResponse save(UUID userId, SaveCardRequest request) {
        ...
        SavedCard duplicate =
                savedCardRepository
                        ...
                        .stream()
                        .filter(card -> isSamePayload(card, request))
                        .findFirst()
                        .orElse(null);
        if (duplicate != null) {
            duplicate.resave(); // 중복은 새로 만들지 않고 최상단으로
            return SavedCardResponse.from(duplicate);
        }

        SavedCard saved = savedCardRepository.save(SavedCard.create(
                userId,
                request.type(),
                request.menuNameKo(),
                request.ingredients(),
    ...
    /** 같은 (user, type, menuNameKo) 후보 중 타입별 페이로드까지 동일하면 중복으로 본다. */
    private boolean isSamePayload(SavedCard card, SaveCardRequest request) {
        return switch (request.type()) {
            case ORDER -> true; // 키(user+type+menuNameKo)만으로 동일
            case INGREDIENT_CHECK, EXCLUDE -> Objects.equals(card.getIngredients(), request.ingredients());
            case OWNER_QUESTION -> Objects.equals(card.getText(), request.text());
        };
    }
